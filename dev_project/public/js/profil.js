document.addEventListener('DOMContentLoaded', function () {
        ...
        setTimeout(() => {
          ...
          $.post(
            ...
            function (ReturnedMessage) {
              if (ReturnedMessage) {
                document.querySelector('.profilAnimal').innerHTML =
                  ReturnedMessage
                document
                  .querySelector('.profilAnimal .iconModif')
  ...
  function openModif(current, action) {
    ...
    current.innerHTML = `<form method="POST">${content[0]}<span id="confirmationMessage"></span></form>`
    ...
    current.querySelector(
      '.innerUsername'
    ).innerHTML = `<input type="text" name="pseudo" value="${content[1]}" id="pseudoSubscribe">
        <label>
            <input type="submit" style="display:none" id="submitModifyAccountPublic">
    ...
    current.querySelector(
      '.desc'
    ).innerHTML = `<textarea name="desc" id="desc">${content[2]}</textarea>`
